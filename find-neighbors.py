        ...
        while disconect_tree:
            ...
            for j in range(1, len(distances[0])):
                ...
                if len(delpoint)!=0:
                    ...
                    if not G.has_edge(str(point), str(per_point)) and min(dic.keys()) < 1.5033:
                        currV = Vertex(point)
...
def buildTree_all_branch1(coor1, coor, trimmedGraph, endNodes_main_artery, start=None):
    ...
    while len(queue[0]):
        ...
        X= np.array(X)
        X = X.tolist()
        distances, ind = tree.query([point], k=4)
        count_neigh = 0
        for j in range(1, len(distances[0])):
            nb_closest = arr[ind[0][j]]

            # if len(X)==1 and distances[0][j] <= 1:
            ...
            # elif len(X)!=0 and nb_closest in X[0] and distances[0][j] <= 1:
            ...
            #         G.add_node(str(pervious_point.point), pos=pervious_point.point)
            #         G.add_edge(str(currV.point), str(pervious_point.point), object=Edge(currV, pervious_point))
            #         queue[0] = np.append(queue[0], [pervious_point.point], axis=0)

            if len(X)!=0 and nb_closest not in X and distances[0][j] <= 2:
...
def writeGraph(G):
    ...
    nx.write_edgelist(G, "graph.csv",  delimiter=' ') #, data=['length', 'radius'] #tree_CIP_right_13feb
...
df1 = pd.read_csv('modify_trunk_mainpul_points_27April.csv', header=None)
...
# df3= pd.read_csv('trunk_CIPdata_1may.csv', header=None) # example for some few branches
df3= pd.read_csv('test_1May.csv', header=None) # example for some few branches
...
df2 = pd.read_csv('only_pulmonary _trunk.csv', header=None) # read the pulmonary trunk
G1 = nx.Graph()
mpa_coor = []
...
for index, row in df1.iterrows():
    coor.append([row[0], row[1], row[2]])
GRAPH = buildTree_all_branch1(coor1, coor, G, endNodes_main_artery, start=None)
